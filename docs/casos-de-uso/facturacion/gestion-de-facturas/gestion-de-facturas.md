...
![Pantalla de facturas](./images/gestion-facturas-1.png)
...
![Pestaña Informes](./images/gestion-facturas-3.png)
...
#### Códigos de color en la gestión de facturas
...
Para realizar una factura de forma manual se pulsa sobre el botón **Nuevo+**, que muestra la pantalla formulario para rellenar con los datos de la factura.
...
## Otras operaciones sobre facturas

### Rectificar una factura

Una rectificación es una factura negativa que se genera a partir de una factura ya cobrada. Para generar una factura rectificativa, se selecciona la factura a rectificar y se pulsa el botón **Rectificar**. Se genera una nueva factura con los mismos datos pero con los importes en negativo. No se pueden hacer rectificaciones parciales.

### Cobrar una factura

Creativa permite cobrar una factura en dos ocasiones:
- Cuando se ha generado de forma manual, y se ha dejado sin marcar como cobrada.
- Cuando se ha generado de forma automática en el proceso de facturación de clientes de crédito.

### Duplicar una factura

Se duplica una factura seleccionada, creando una nueva con los mismos datos pero sin número de factura asignado. Es útil para crear facturas similares a otras ya creadas, evitando tener que rellenar todos los datos de nuevo.

Para duplicar una factura, se selecciona la factura a duplicar y se pulsa el botón **Duplicar**. Se genera una nueva factura con los mismos datos pero sin número de factura asignado.

A continuación, se pueden modificar los datos que se deseen y guardar la nueva factura. La factura original permanece sin cambios.

Por último se utiliza el botón **Cobrar factura** para generar un movimiento de caja y marcar la nueva factura como cobrada, o se pueden agregar entregas a cuenta para cubrir el importe de la factura.

### Eliminar facturas

Eliminar una factura es una acción que no se puede deshacer, por lo que se recomienda utilizarla con precaución. Para eliminar una factura, se selecciona la factura o facturas a eliminar y se pulsa el botón **Borrar**. Se muestra un mensaje de confirmación para evitar eliminaciones accidentales.

En el caso de que esté activo Veri*factu o SII, no se podrán eliminar facturas que ya hayan sido enviadas a la AEAT.

## Facturación mensual

Creativa permite generar facturas de forma automática a clientes de crédito, con una periodicidad mensual. Para ello, se utiliza el proceso de facturación mensual, que se encuentra en el menú **Facturación > Facturación mensual**.
